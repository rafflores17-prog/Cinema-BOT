...
def build_caption(details, is_tv=False):
    """Caption organizado — tudo em linhas limpas sem quebra no meio das infos."""
    if not details:
    ...
    # Limite maior para não cortar demais
    if len(sinopse) > 380: sinopse = sinopse[:380].rsplit(" ", 1)[0] + "…"
    ...
    if is_tv:
        ...
        duracao = f"{seasons} temp. • {episodes} ep." if seasons else ""
    else:
        runtime = details.get("runtime") or 0
        duracao = formatar_runtime(runtime)
        status  = ""

    # Elenco top 4 (menos nomes = linha mais curta)
    credits   = details.get("credits", {})
    cast_list = [p.get("name","") for p in credits.get("cast", [])[:4]]
    ...
    # ── Nota formatada numa só linha ──────────────────────────────────────
    estrelas   = formatar_estrelas(rating)
    nota_linha = f"{estrelas} <b>{rating:.1f}/10</b>"
    if count:
        # Formata votos sem vírgula americana — usa ponto br
        votos_fmt  = f"{count:,}".replace(",", ".")
        nota_linha += f" <i>({votos_fmt} votos)</i>"

    # ── Linha de metadados: ano • duração • gêneros ───────────────────────
    meta_parts = [ano]
    if duracao:   meta_parts.append(duracao)
    if generos:   meta_parts.append(generos)
    meta_linha = " • ".join(meta_parts)
    ...
    # Cabeçalho
    L.append(f"{icone} <b>{html.escape(titulo)}</b>")
    if orig_title and orig_title != titulo:
        L.append(f"<i>{html.escape(orig_title)}</i>")

    # Metadados numa linha
    L.append(f"📅 {html.escape(meta_linha)}")
    ...
    # Temporadas/eps série
    if is_tv and duracao:
        L.append(f"📺 {html.escape(duracao)}")

    # Separador
    L.append("")

    # Sinopse
    L.append(f"📖 {html.escape(sinopse)}")
    ...
    # Elenco e direção
    if elenco:
        L.append(f"🌟 <b>Elenco:</b> {html.escape(elenco)}")
    if direcao_val:
        L.append(f"{direcao_label} <b>Direção/Criação:</b> {html.escape(direcao_val)}")
...
async def send_item(context, item, is_tv=False, tipo="movie"):
    ...
    # Botões organizados: ASSISTIR grande, depois TRAILER e SITE lado a lado
    keyboard = [
        [InlineKeyboardButton("▶️  ASSISTIR AGORA", url=url_direto)],
        [
            InlineKeyboardButton("🎬 Ver Trailer", url=url_trailer),
            InlineKeyboardButton("🌐 Visite o Site", url=SITE_URL)
        ]
    ]
    ...
    try:
        ...
        # 2) Trailer — se tiver link direto: botão "Ver no YouTube" + mini player
        if "youtu.be/" in url_trailer or "youtube.com/watch" in url_trailer:
            trailer_kb = InlineKeyboardMarkup([
                [InlineKeyboardButton("▶️ Ver Trailer no YouTube", url=url_trailer)]
            ])
            await enviar(context, text="\U0001f3ac <b>Trailer disponível:</b>\n" + url_trailer,
                         markup=trailer_kb)
